...
/**
 * Public Page rendering (Phase 4 / Task 4.4 / ADR-017 / ADR-025).
 */
class PageController extends BaseController
{
    ...
    {
        $path = $this->publicUrlBuilder()->pagePath($slug, $locale);
        $redirectTarget = $this->urlRedirectService()->findActiveTarget($path);
        if ($redirectTarget !== null) {
            return redirect()->to($redirectTarget, 301);
        }

        $package = $this->pageService()->findPublishedPackageForPublic($slug, $locale);
        if ($package === null) {
            throw PageNotFoundException::forPageNotFound();
        }
